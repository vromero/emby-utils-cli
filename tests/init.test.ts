...
describe("runInit", () => {
  it("runs the full wizard then adds new libraries on a fresh server", async () => {
    ...
    expect(result.libraries).toEqual([
      { name: "Movies", status: "created" },
      { name: "TV", status: "created" },
    ]);
  ...
  it("is idempotent: existing libraries are skipped on a re-run", async () => {
    ...
    expect(result.libraries).toEqual([{ name: "Movies", status: "skipped" }]);
  });
  ...
  it("creates requested API keys and returns their tokens", async () => {
    ...
    expect(
      result.apiKeys
        .filter((k) => k.status === "created")
        .every((k) => /^generated-token-\d+$/.test(k.token))
    ).toBe(true);
  ...
  it("is idempotent: existing API keys are reused and not recreated", async () => {
    ...
    expect(result.apiKeys.filter((k) => k.status === "created")).toEqual([]);
    expect(result.apiKeys).toEqual([
      { app: "my-app", token: "existing-token", status: "skipped" },
    ]);
    // No mutation attempted.
    expect(postCount).toBe(0);
  });
  ...
  it("mixes created and skipped keys in a single run", async () => {
    ...
    expect(result.apiKeys).toEqual([
      { app: "existing", token: "existing-token", status: "skipped" },
      { app: "fresh", token: "new-token-1", status: "created" },
    ]);
  });
...
describe("emby init (CLI)", () => {
  it("runs through the full flow end-to-end from a config file", async () => {
    ...
    try {
      ...
      expect(out.libraries).toEqual([
        { name: "Movies", status: "created" },
        { name: "TV", status: "created" },
      ]);
      expect(out.accessToken).toBe("new-token");
      const created = out.apiKeys.filter((k: any) => k.status === "created");
      expect(created).toHaveLength(1);
      expect(created[0].app).toBe("my-app");
      expect(created[0].token).toMatch(/^generated-token-\d+$/);
      expect(out.apiKeys.filter((k: any) => k.status === "skipped")).toEqual([]);
